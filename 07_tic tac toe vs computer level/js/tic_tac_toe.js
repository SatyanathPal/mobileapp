...
// Array に変換…もし line28 で getElementsByClassName を使う場合は、
// forEach が使えないので Array にする
// const levelsArray = Array.from(levels);

// level の要素を取得　レベル設定エリア
...
const msgtxt1 = '<p class="image"><img src="img/penguins.jpg" width=61 height=61></p><p class="text">Penguins Attack! (you turn)</p>';
const msgtxt2 = '<p class="image"><img src="img/whitebear.jpg" width=61 height=61></p><p class="text">WhiteBear Attack! (computer turn)</p>';
const msgtxt3 = '<p class="image animate__animated animate__bounce"><img src="img/penguins.jpg" width=61 height=61></p><p class="text animate__animated animate__tada">Penguins Win!</p>';
const msgtxt4 = '<p class="image animate__animated animate__bounce"><img src="img/whitebear.jpg" width=61 height=61></p><p class="text animate__animated animate__tada">WhiteBear Wins!</p>';
const msgtxt5 = '<p class ="image"><img src="img/penguins.jpg" width=61 height=61><img src="img/whitebear.jpg" width=61 height=61></p><p class="text animate__animated animate__flash">Draw!!</p>';
...
function LevelSetting(index) {
  ...
  // セッションストレージに key="tic_tac_toe_access" がある場合は初回ではない
  if (sessionStorage.getItem("tic_tac_toe_access")) {
    ...
    // セッションストレージに key="tic_tac_toe_access" がない場合は初回実行
    sessionStorage.setItem("tic_tac_toe_access", "1");
    level_1.classList.add("level-selected");
    level_2.classList.add("level-non-selected");
    level_3.classList.add("level-non-selected");
...
function isReach(status) {
  ...
  lineArray.some(function (line) {
    ...
    // クマ or ペンギンのリーチ行ありの場合、空いているマス目を選択する
    if (bearTurnEnd === "1") {
      ...
      line.some(function (square) {
        ...
        if (square.classList.contains("js-clickable")) {
          isSelect(square);
          return true;   // line.some() の loop をぬける
        }

      });

      return true;       // lineArray.some() の loop をぬける
    }
